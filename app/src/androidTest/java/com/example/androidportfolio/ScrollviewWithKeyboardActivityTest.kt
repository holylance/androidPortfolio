...
class ScrollviewWithKeyboardActivityTest {
  ...
  fun firstLayout() {
    ...
    onView(withText(R.string.desc_scrollview_with_keyboard)).check(matches(isDisplayed()))
    onView(withHint(R.string.edit_hint)).check(matches(isDisplayed()))
    onView(withText(R.string.button)).check(matches(isDisplayed()))
    onView(withText(R.string.desc_dummy_text_to_scroll)).check(matches(isDisplayed()))
  }
